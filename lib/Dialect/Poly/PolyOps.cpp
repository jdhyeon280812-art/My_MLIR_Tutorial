...
OpFoldResult MulOp::fold(MulOp::FoldAdaptor adaptor) {
  auto lhsAtrr = adaptor.getOperands()[0];
  auto rhsAtrr = adaptor.getOperands()[1];

  if(!lhsAtrr || !rhsAtrr)
    return {};

  auto lhs = dyn_cast<DenseIntElementsAttr>(lhsAtrr);
  auto rhs = dyn_cast<DenseIntElementsAttr>(rhsAtrr);
